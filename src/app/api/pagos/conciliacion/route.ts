...
const reconcileSchema = z.object({ movimientoId: z.string().min(1), pagoId: z.string().min(1) })
...
export const POST = withAuth(async (req, session) => {
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      const updatedMovement = await tx.movimientoExtracto.update({ where: { id: movement.id }, data: { pagoId: payment.id, estado: "CONCILIADO", conciliadoPorId: session.user.id, conciliadoAt: new Date() } })
      const updatedPayment = await tx.pago.update({ where: { id: payment.id }, data: { conciliado: true, estado: "CONCILIADO" } })
