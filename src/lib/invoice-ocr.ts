...
export interface InvoiceTaxDraft {
  type: "IVA" | "RECARGO_EQUIVALENCIA" | "IRPF"
  percentage: string
  taxableBase: string
  taxAmount: string
}
...
export interface InvoiceDraft {
  ...
  totalAmount: string
  notes: string
  validRecipientTaxId: boolean
  lines: InvoiceLineDraft[]
  taxes: InvoiceTaxDraft[]
}
...
const RECIPIENT_TAX_ID_DIGITS = "09711078"

function matchesRecipientTaxId(text: string): boolean {
  const cleaned = normalizeAlpha(text)
  if (cleaned.includes("b09711078")) return true
  const idx = cleaned.indexOf(RECIPIENT_TAX_ID_DIGITS)
...
function extractInvoiceNumber(lines: string[]) {
  ...
  for (const line of lines) {
    ...
    const genericInvoice = line.match(/^\s*factura\b(.+)$/i)
    if (genericInvoice) {
      const candidate = firstInvoiceCandidate(genericInvoice[1])
      if (candidate) return candidate
    }
  }
...
function isRecipientTaxId(value: string) {
  ...
  return normalized === RECIPIENT_TAX_ID_DIGITS || normalized === `b${RECIPIENT_TAX_ID_DIGITS}` || normalized === `esb${RECIPIENT_TAX_ID_DIGITS}`
}
...
function parseYolmarLine(rawLine: string, nextLine: string) {
  ...
  const draft = emptyInvoiceLine()
  draft.supplierReference = reference
  draft.description = description.trim()
  draft.unitOfMeasure = unit
  draft.quantity = amount(quantity)
  draft.discountPercentage = amount(discount)
  draft.unitPrice = amount(price)
  draft.netUnitPrice = amount(price)
  draft.taxableBase = amount(lineTotal)
  draft.vatRate = amount(tax)
  draft.vatAmount = amount(taxAmount)
  draft.lineTotal = amount(lineTotal)
  draft.batch = nextLine.match(/lote:\s*([^\s]+)/i)?.[1] || ""
  draft.dueDate = parseDateInLine(nextLine)
  return draft
}
...
function parseGenericLine(line: string) {
  ...
  draft.supplierReference = reference
  draft.description = description.trim()
  draft.originalFormat = line.trim()
  draft.quantity = amount(quantity)
  draft.unitPrice = amount(price)
  draft.netUnitPrice = amount(price)
  draft.taxableBase = amount(subtotal)
  draft.vatRate = amount(tax)
  draft.lineTotal = amount(subtotal)
  return draft
}
...
function parseLacteosLine(line: string) {
  ...
  draft.supplierReference = reference
  draft.description = description.trim()
  draft.unitOfMeasure = format.trim().split(/\s+/)[0] || ""
  draft.originalFormat = format.trim()
  draft.quantity = amount(quantity)
  draft.unitPrice = amount(unitPrice)
  draft.netUnitPrice = amount(unitPrice)
  draft.taxableBase = amount(subtotal)
  draft.vatRate = amount(tax)
  draft.vatAmount = (Number(amount(subtotal)) * Number(amount(tax)) / 100).toFixed(2)
  draft.lineTotal = amount(subtotal)
  draft.batch = lot
  draft.dueDate = parseDateInLine(expiry)
  return draft
}
...
function addTaxRow(rows: InvoiceTaxDraft[], type: InvoiceTaxDraft["type"], percentage: number, base: string, quota: string) {
  const normalizedPercentage = percentage.toFixed(2)
  const existing = rows.find((row) => row.type === type && row.percentage === normalizedPercentage)
  if (!existing) {
    rows.push({ type, percentage: normalizedPercentage, taxableBase: amount(base), taxAmount: amount(quota).replace("-", "") })
    return
  }
  if (existing.taxableBase === amount(base) && existing.taxAmount === amount(quota).replace("-", "")) return
  existing.taxableBase = (Number(existing.taxableBase) + Number(amount(base))).toFixed(2)
  existing.taxAmount = (Number(existing.taxAmount) + Number(amount(quota).replace("-", ""))).toFixed(2)
}
...
function parseTaxRows(lines: string[]) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (rateMatch && (isTaxLine || inTaxBlock)) {
      ...
      const percentage = isWithholdingTaxLine ? Number(rateMatch[1].replace(",", ".")) : taxRate(rateMatch[1])
      if (percentage == null) continue
      if (isWithholdingTaxLine) {
...
function sumTaxRows(rows: InvoiceTaxDraft[], type: InvoiceTaxDraft["type"]) {
  return rows.filter((row) => row.type === type).reduce((sum, row) => sum + Number(row.taxAmount), 0).toFixed(2)
}

function sumTaxBases(rows: InvoiceTaxDraft[]) {
  return rows.filter((row) => row.type === "IVA").reduce((sum, row) => sum + Number(row.taxableBase), 0).toFixed(2)
}
...
function parseIkeaReceiptLine(lines: string[], index: number) {
  ...
  const draft = emptyInvoiceLine()
  draft.lineType = "PRODUCTO"
  draft.supplierReference = refMatch[1]
  draft.itemCode = refMatch[2] || ""
  ...
  draft.description = descLine.replace(/[“”"]/g, "").replace(/\s+\d+$/, "").replace(/\bc(?:art|ort) opac 2[0o]\b/i, "cort opac 2u").replace(/\bSTRIMWIG\b/i, "STRIMMIG").replace(/\bTREA\s+965\+/i, "IKEA 365+").replace(/salv?nantéiman/i, "salvamant&imán").replace(/breia bbsoa elieonas/i, "brcha bbcoa silicona").replace(/\s{2,}/g, " ").trim()
  draft.quantity = "1.00"
  const nums = amounts(priceLine)
  const hasDecimal = /\d+[,.]\d+/.test(priceLine)
  if (nums.length >= 3) {
    draft.quantity = nums[0]
    draft.unitPrice = nums[1]
    draft.netUnitPrice = nums[1]
    draft.taxableBase = nums[2]
  } else if (nums.length === 2 && hasDecimal) {
    draft.quantity = "1.00"
    draft.unitPrice = nums[0]
    draft.netUnitPrice = nums[0]
    draft.taxableBase = nums[1] === "0.00" ? nums[0] : nums[1]
  } else if (nums.length === 1 && hasDecimal) {
    draft.unitPrice = nums[0]
    draft.netUnitPrice = nums[0]
    draft.taxableBase = nums[0]
  }
  draft.vatRate = "21.00"
  draft.lineTotal = draft.taxableBase
  return draft
}
...
export function parseInvoiceText(text: string): InvoiceDraft {
  ...
  draft.issuerTaxId = findIssuerTaxId(text, lines)
  draft.validRecipientTaxId = matchesRecipientTaxId(text)

  const number = extractInvoiceNumber(lines)
  const split = splitInvoiceNumber(number)
  draft.series = split.series.toUpperCase()
  draft.number = split.number.toUpperCase()
  draft.issueDate = findDate(lines, ["fecha de factura", "fecha factura", "fecha de emisión", "fecha emisión", "fecha de expedición", "fecha expedición", "fecha de expedicion", "fecha expedicion", "invoice date", "factura núm", "factura num"])
  if (!draft.issueDate) {
  ...
  draft.operationDate = findDate(lines, ["fecha de operación", "fecha operacion"])
  draft.dueDate = findDate(lines, ["vencimiento"])
  draft.paymentDate = findDate(lines, ["fecha de pago"])
  draft.paymentMethod = findPaymentMethod(lines) || findValue(lines, ["forma de pago", "forma pago"])
  if (!draft.paymentMethod && normalized.some((line) => line.includes("transferencia bancaria"))) draft.paymentMethod = "Transferencia bancaria"
  if (!draft.paymentMethod && normalized.some((line) => /\befectivo\b/.test(line))) draft.paymentMethod = "Efectivo"
  ...
  draft.orderNumber = orderNumber?.[1] || ""
  draft.orderDate = orderDate ? parseDateInLine(orderDate[1]) : ""
  draft.deliveryCenter = deliveryCenter?.[1]?.trim() || ""

  const deliveryNoteLine = lines.find((line) => /alb[-_]?\d{4}\s*\/\s*\d+/i.test(line)) || ""
  const deliveryNoteMatch = deliveryNoteLine.match(/(\d{1,2}[/-]\d{1,2}[/-]\d{2,4})\s+([A-Z0-9_-]+)\s*\/\s*(\d+)/i) || deliveryNoteLine.match(/([A-Z0-9_-]+)\s*\/\s*(\d+)/i)
  if (deliveryNoteMatch) {
    const hasDate = deliveryNoteMatch.length === 4
    draft.deliveryNoteDate = hasDate ? parseDateInLine(deliveryNoteMatch[1]) : ""
    const series = hasDate ? deliveryNoteMatch[2] : deliveryNoteMatch[1]
    const number = hasDate ? deliveryNoteMatch[3] : deliveryNoteMatch[2]
    draft.deliveryNoteReference = `${series}/${number}`
  }

  draft.issuerLegalName = findIssuerName(lines, draft.issuerTaxId)

  if (draft.issuerTaxId) {
    const taxIdClean = normalize(draft.issuerTaxId).replace(/-/g, "")
    const taxIdIndex = normalized.findIndex((line) => line.replace(/-/g, "").includes(taxIdClean))
    if (taxIdIndex >= 0) {
      const nearbyAddress = lines.slice(taxIdIndex + 1, taxIdIndex + 6).filter((line) => /(?:calle|c\/|av\.|avenida|^\d{5})/i.test(line))
      draft.issuerBillingAddress = nearbyAddress.join(", ").replace(/\bn[*º]?\s*9\b/i, "nº9")
    }
  }
  ...
  if (!draft.issuerTaxId) {
    ...
    if (cifLine) {
      ...
      if (cifMatch) draft.issuerTaxId = cifMatch[1].replace(/\s/g, "").toUpperCase()
    }
  }

  if (!draft.issuerBillingAddress) {
    ...
    if (parts.length) draft.issuerBillingAddress = parts.map((p) => p.trim()).join(", ")
  }

  const taxRows = parseTaxRows(lines)
  for (const row of [...parseImplicitProductTaxes(lines), ...parseIkeaTaxRows(lines)]) {
    addTaxRow(taxRows, row.type, Number(row.percentage), row.taxableBase, row.taxAmount)
  }
  const subjectBase = findSameLineAmount(lines, ["sujeto a iva"])
  const bareVat = findLastSameLineAmount(lines, ["i.v.a."])
  const bareVatRate = lines.find((line) => /i\.?v\.?a\.?\s*(?:\(?\s*)?(\d+(?:[,.]\d+)?)\s*%/i.test(line))?.match(/(\d+(?:[,.]\d+)?)\s*%/)?.[1]
  if (subjectBase !== "0" && bareVat !== "0" && bareVatRate) addTaxRow(taxRows, "IVA", Number(bareVatRate.replace(",", ".")), subjectBase, bareVat)
  draft.taxes = taxRows
  ...
  const taxBase = sumTaxBases(draft.taxes)
  ...
  draft.netTotal = explicitNetSameLine !== "0" ? explicitNetSameLine : taxBase !== "0.00" && !trustedNetAfterLabel ? taxBase : netAfterLabel !== "0" ? netAfterLabel : taxBase !== "0.00" ? taxBase : baseOnLabel

  const vatRows = draft.taxes.filter((row) => row.type === "IVA")
  const missingBaseRows = vatRows.filter((row) => Number(row.taxableBase) === 0)
  const knownTaxBase = sumTaxBases(draft.taxes)
  if (missingBaseRows.length === 1 && Number(draft.netTotal) > Number(knownTaxBase)) {
    missingBaseRows[0].taxableBase = (Number(draft.netTotal) - Number(knownTaxBase)).toFixed(2)
  }

  draft.discountTotal = findSameLineAmount(lines, ["total descuento", "descuento total"])
  if (draft.discountTotal === "0") draft.discountTotal = "0.00"

  const explicitVatSameLine = findSameLineAmount(lines, ["total iva + re", "total iva", "total impuesto", "importe iva", "importe i.v.a.", "impuesto aplicado", "i.v.a. / i.g.i.c."])
  const explicitVat = explicitVatSameLine !== "0" ? explicitVatSameLine : findLastSameLineAmount(lines, ["i.v.a."])
  const taxAmount = sumTaxRows(draft.taxes, "IVA")
  draft.totalVat = taxAmount !== "0.00" ? taxAmount : explicitVat
  draft.surchargeTotal = findSameLineAmount(lines, ["total re", "total recargo"])
  const retained = sumTaxRows(draft.taxes, "IRPF")
  draft.withholdingTotal = retained !== "0.00" ? retained : findSameLineAmount(lines, ["total irpf", "retenciones", "retencion"])
  ...
  draft.totalAmount = explicitTotal
  const calculatedTotal = Number(draft.netTotal) + Number(draft.totalVat) - Number(draft.withholdingTotal)
  if (draft.totalAmount === "0" && calculatedTotal > 0 && (Number(draft.netTotal) > 0 || Number(draft.totalVat) > 0)) {
    draft.totalAmount = calculatedTotal.toFixed(2)
  }

  if (draft.totalAmount === "0") {
    const totalAfterLabel = findLabelAmount(lines, totalLabels)
    if (totalAfterLabel !== "0") draft.totalAmount = totalAfterLabel
  }
  ...
  if (draft.totalAmount === "0") {
    for (let index = lines.length - 1; index >= 0; index -= 1) {
      ...
      if (values.length) { draft.totalAmount = values[values.length - 1]; break }
    }
  }
  ...
  if (draft.totalAmount === "0") {
    ...
    if (lastStandaloneAmount !== "0") draft.totalAmount = lastStandaloneAmount
  }

  if (draft.totalAmount === "0") {
    const cashIdx = normalized.findIndex((line) => /efectivo|tarjeta|pago/.test(line))
    for (let index = cashIdx - 1; cashIdx > 0 && index >= 0; index -= 1) {
      const values = numericValues(lines[index])
      if (values.length && values[values.length - 1] !== "0.00") { draft.totalAmount = values[values.length - 1]; break }
    }
  }
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (parsed) draft.lines.push(parsed)
  }
  if (draft.lines.length > 1 && draft.lines[0].description === "") draft.lines.shift()
  if (hasIkeaReceipt && draft.totalAmount !== "0" && draft.lines.length) {
    const missingLines = draft.lines.filter((line) => Number(line.lineTotal) === 0 || Number(line.unitPrice) === 0)
    if (missingLines.length >= 1) {
      const knownTotal = draft.lines.reduce((sum, line) => sum + Number(line.lineTotal), 0)
      const remainder = Number(draft.totalAmount) - knownTotal
      if (remainder > 0) {
        const missing = missingLines[0]
        missing.unitPrice = remainder.toFixed(2)
        missing.netUnitPrice = remainder.toFixed(2)
        missing.taxableBase = remainder.toFixed(2)
        missing.lineTotal = remainder.toFixed(2)
      }
    }
  }
  if (draft.lines.length === 1 && !draft.lines[0].description && /art[ií]culo|albar[aá]n|producto\s+servido|gtin|c[oó]digo\s+ean|descripci[oó]n\s+del\s+art[ií]culo|detalles\s+de\s+la\s+factura|\[(?:pjs|delivery)_/i.test(normalized.join(" "))) {
    draft.lines[0].description = "COMPRA"
  }
  return draft
}
