...
function ProductoCombobox({ id, productos, selectedProducto, value, onSelect, error = false }: { id?: string; productos: Producto[]; selectedProducto?: Producto; value: string; onSelect: (productoId: string) => void; error?: boolean }) {
  ...
  return <div ref={containerRef} className="relative w-full">
    <input id={id} type="text" role="combobox" readOnly value={selected ? `${selected.codigo} - ${selected.descripcionTpv}` : ""} placeholder="Buscar producto..." onFocus={() => setOpen(true)} onClick={() => setOpen(true)} aria-expanded={open} aria-haspopup="listbox" aria-controls={id ? `${id}-options` : undefined} aria-invalid={error || undefined} className={`w-full cursor-pointer rounded-md border px-2 py-1.5 text-xs text-gray-900 focus:outline-none focus:ring-1 ${error ? "border-red-500 bg-red-50 focus:border-red-500 focus:ring-red-500" : "border-gray-300 focus:border-blue-500 focus:ring-blue-500"}`} />
...
async function extractDocument(file: File, setStatus: (value: string) => void) {
  if (!(await isPdfFile(file))) return recognizeImage(file, setStatus)
  ...
  return (
    <form onSubmit={async (event) => { event.preventDefault(); if (!missingRequired) await onSubmit(data) }} className="space-y-5">
      ...
      <section id="factura-lineas">
         <div className="mb-2 flex flex-wrap items-center justify-between gap-2"><h3 className="text-sm font-semibold text-gray-900">Líneas</h3><div className="flex gap-2"><button type="button" onClick={() => addLine("PRODUCTO")} className="rounded-md border px-2 py-1 text-xs text-gray-700">+ Producto</button><button type="button" onClick={() => addLine("CARGO")} className="rounded-md border px-2 py-1 text-xs text-gray-700">+ Cargo</button></div></div>
      ...
      {data.cifReceptor !== "B09711078" && <p className="rounded-md bg-red-50 p-2 text-xs text-red-700">CIF receptor incorrecto. Debe ser B09711078.</p>}
       <div className="flex gap-2"><button type="submit" disabled={missingRequired || saving} className="rounded-md bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700 disabled:opacity-50">{saving ? "Guardando..." : missingRequired ? `Faltan ${missingFields.length} campos` : "Confirmar factura"}</button><button type="button" onClick={onCancel} className="rounded-md border px-4 py-2 text-sm text-gray-700">Cancelar</button></div>
    </form>
  )
}
