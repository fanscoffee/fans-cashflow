...
export default function PagosPanel() {
  ...
  return (
    <div className="pagos-panel min-w-0 space-y-6">
      ...
      {(dashboard?.pendingExpenses || []).length > 0 && <section className="rounded-lg border bg-white p-4 shadow-sm sm:p-6"><h2 className="mb-3 break-words text-lg font-semibold text-gray-900 [overflow-wrap:anywhere]">Autorizaciones pendientes · gastos de turnos</h2><div className="space-y-2">{dashboard?.pendingExpenses.map((expenseItem) => <div key={expenseItem.id} className="flex flex-col gap-3 rounded-md border p-3 text-sm sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><p className="break-words font-medium text-gray-900 [overflow-wrap:anywhere]">{expenseItem.concepto}</p><p className="break-words text-xs text-gray-500 [overflow-wrap:anywhere]">{expenseItem.entidad} · {expenseItem.categoria?.nombre} · {euros(expenseItem.importe)} · solicitado por {expenseItem.solicitante?.name || expenseItem.solicitante?.email || "Usuario desconocido"}</p></div><div className="grid grid-cols-2 gap-2 sm:flex"><button type="button" disabled={saving} onClick={() => void authorizeExpense(expenseItem.id, true)} className="min-h-11 rounded-md bg-green-700 px-3 py-1.5 text-xs font-medium text-white disabled:opacity-50 sm:min-h-0">Autorizar</button><button type="button" disabled={saving} onClick={() => void authorizeExpense(expenseItem.id, false)} className="min-h-11 rounded-md border border-red-200 px-3 py-1.5 text-xs font-medium text-red-700 disabled:opacity-50 sm:min-h-0">Rechazar</button></div></div>)}</div></section>}
    </div>
  )
}
